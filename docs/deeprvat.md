...
- `genotypes.h5`
contains the genotypes for all samples in a custom sparse format. The sample ids in the `sample` slot are the same as in the VCF files the `genotypes.h5` has been read from. 
...
is output from the input vcf files using preprocessing pipeline. Instructions [here](https://deeprvat.readthedocs.io/en/latest/preprocessing.html).
...
Maps the `gene_id` used in `annotations.parquet` to actual genes (EnsemblID and HGNC gene name). This is an output from the annotation pipeline. Instructions [here](https://deeprvat.readthedocs.io/en/latest/annotations.html).

main() {
    ...
    mkdir -p $phenotype/deeprvat/burdens_chunk_$chunk
    touch fakeburdens.zarr
    python deeprvat/deeprvat/deeprvat/associate.py compute-burdens \
           --n-chunks $n_chunks --chunk $chunk \
           --dataset-file $BASE/workdir/500k_run2/$phenotype/deeprvat/association_dataset.pkl \
           --link-burdens fakeburdens.zarr \
           $BASE/workdir/500k_run2/$phenotype/deeprvat/hpopt_config.yaml \
           ...
           $phenotype/deeprvat/burdens_chunk_$chunk

    echo "Uploading outputs"
    echo "rm config.yaml ..."
    rm -rf deeprvat config.yaml genotypes.h5 variants.parquet variants_90pct10dp_qc.parquet phenotypes.parquet annotations.parquet protein_coding_genes.parquet
    echo "tar cf $phenotype/deeprvat/burdens_chunk_${chunk}.tar $phenotype/deeprvat/burdens_chunk_$chunk"
    tar cf $phenotype/deeprvat/burdens_chunk_${chunk}.tar $phenotype/deeprvat/burdens_chunk_$chunk
    echo "rm -rf $phenotype/deeprvat/burdens_chunk_$chunk"
    rm -rf $phenotype/deeprvat/burdens_chunk_$chunk

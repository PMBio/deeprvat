main() {
    ...
    echo "Packing output"
    output_path=$(echo $command  | rev | cut -d' ' -f 1 | rev)
    echo tar cf "$output_path/burdens.zarr.tar" "$output_path/burdens.zarr"
    echo tar cf "$output_path/samples.zarr.tar" "$output_path/samples.zarr"

    tar cf "$output_path/burdens.zarr.tar" "$output_path/burdens.zarr"
    tar cf "$output_path/samples.zarr.tar" "$output_path/samples.zarr"

    echo rm -r "$output_path/burdens.zarr"
    echo rm -r "$output_path/samples.zarr"

    rm -r "$output_path/burdens.zarr"
    rm -r "$output_path/samples.zarr"

...
def splicemap_cat5(wildcards):
    ...
    return (
        path
        / f"{wildcards.tissue_cat}_splicemap_psi5_method=kn_event_filter=median_cutoff.csv.gz"
    )
...
def splicemap_cat3(wildcards):
    ...
    return (
        path
        / f"{wildcards.tissue_cat}_splicemap_psi3_method=kn_event_filter=median_cutoff.csv.gz"
    )
...
rule all_predict_rna:
    input:
        expand(
            config_pred["splicing_pred"]["delta_psi_inferred_from_cat"],
            genome=absplice_main_conf["genome"],
            vcf_id=vcf_ids,
            tissue_cat=absplice_main_conf["DROP"]["DROP_group"],
        ),
        expand(
            config_pred["splicing_pred"]["absplice_rna"],
            genome=absplice_main_conf["genome"],
            vcf_id=vcf_ids,
            tissue_cat=absplice_main_conf["DROP"]["DROP_group"],
        ),
        expand(
            config_pred["splicing_pred"]["absplice_rna_with_dna_info"],
            genome=absplice_main_conf["genome"],
            vcf_id=vcf_ids,
            tissue_cat=absplice_main_conf["DROP"]["DROP_group"],
        ),

...
pvcf_blocks_by_chr = pvcf_blocks_df.groupby("chr_name")
...
rule preprocess:
    input:
        ...
        sparse_gt = lambda wildcards: [
            sparse_dir / "chr{chr}" / f"{vcf_filename_pattern}.tsv.gz".replace("{block}", str(b))
            for b in pvcf_blocks_by_chr.get_group(wildcards.chr)["Block"]
        ],
        # expand(
        #     sparse_dir / "chr{{chr}}" / f"{vcf_filename_pattern_no_chr_expand}.tsv.gz",
        #     zip,
        #     # chr=chromosomes,
        #     block=block,
        # ),
        qc_varmiss = lambda wildcards: [
            qc_varmiss_dir / f"{vcf_filename_pattern}.tsv.gz".replace("{block}", str(b))
            for b in pvcf_blocks_by_chr.get_group(wildcards.chr)["Block"]
        ],
        # expand(
        #     qc_varmiss_dir / f"{vcf_filename_pattern_no_chr_expand}.tsv.gz".
        #     zip,
        #     # chr=chromosomes,
        #     block=block,
        # ),
        qc_hwe = lambda wildcards: [
            qc_hwe_dir / f"{vcf_filename_pattern}.tsv.gz".replace("{block}", str(b))
            for b in pvcf_blocks_by_chr.get_group(wildcards.chr)["Block"]
        ],
        # expand(
        # qc_hwe_dir / f"{vcf_filename_pattern_no_chr_expand}.tsv.gz"
        #     zip,
        #     # chr=chromosomes,
        #     block=block,
        # ),
        qc_read_depth = lambda wildcards: [
            qc_read_depth_dir / "chr{chr}" / f"{vcf_filename_pattern}.tsv.gz".replace("{block}", str(b))
            for b in pvcf_blocks_by_chr.get_group(wildcards.chr)["Block"]
        ],
        # expand(
        #     qc_read_depth_dir / "chr{{chr}}" / f"{vcf_filename_pattern_no_chr_expand}.tsv.gz"
        #     zip,
        #     # chr=chromosomes,
        #     block=block,
        # ),
        qc_allelic_imbalance = lambda wildcards: [
            qc_allelic_imbalance_dir / f"{vcf_filename_pattern}.tsv.gz".replace("{block}", str(b))
            for b in pvcf_blocks_by_chr.get_group(wildcards.chr)["Block"]
        ],
        # expand(
        #     qc_allelic_imbalance_dir / f"{vcf_filename_pattern_no_chr_expand}.tsv.gz"
        #     zip,
        #     # chr=chromosomes,
        #     block=block,
        # ),

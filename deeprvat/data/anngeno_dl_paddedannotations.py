...
class AnnGenoDataset:
    ...
    def __getitem__(self, idx: int) -> Dict[str, np.ndarray]:
        ...
        # samples x regions x annotations x variants
        max_variants = 0
        regions_nonzero_sample = []
        ...
        for r in regions:
            ...
            max_variants = max(
                max_variants, np.max(np.sum(this_region["genotypes"], axis=1))
            )
            this_nonzero = np.nonzero(this_region["genotypes"])
            regions_nonzero_sample.append(this_nonzero[0])
            regions_nonzero_variant.append(this_nonzero[1])
            regions_genotypes.append(this_region["genotypes"][this_nonzero])
            annotations.append(this_region["annotations"])
            del this_region

        n_samples = sample_slice.stop - sample_slice.start
        n_regions = len(self.regions)
        n_annotations = len(annotations[0].columns)
        variants_set = np.zeros((n_samples, n_regions), dtype=np.uint32)
        ...
        for region, anno in enumerate(annotations):
            for sample, variant, genotype in zip(
                ...
                regions_genotypes[region],
            ):
                for _ in range(genotype):
                    # set corresponding element of padded_annotations
                    padded_annotations[
                        sample, region, :, variants_set[sample, region]
                    ] = anno.iloc[variant]
                    variants_set[sample, region] += 1

        result["padded_annotations"] = padded_annotations

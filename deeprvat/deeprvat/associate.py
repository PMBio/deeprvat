...
def make_skat_input_(
    ...
    annotation_cols = data_config["association_testing_data"]["dataset_config"][
        "rare_embedding"
    ]["config"]["annotations"]
    variant_file = data_config["association_testing_data"]["variant_file"]
    ...
    variants = pd.merge(
        annotations,
        pd.read_parquet(variant_file, columns=["id", "chrom", "pos", "ref", "alt"]),
    )
    for op in thresholds.values():
        variants = variants.query(op)

    if variants["gene_id"].dtype == object:
        variants = (
            variants.loc[variants["gene_id"].apply(len) > 0]
            .explode("gene_id")
            .astype({"gene_id": int})
        )

    variants = variants.set_index(["gene_id", "id"])

    logger.info("Scoring all rare variants")
    scores = score_variants(
        variants[annotation_cols], model_config_file, data_config_file, checkpoint_files
    )

    variants["chrom_pos_ref_alt"] = variants["chrom"].str.cat(
        variants[["pos", "ref", "alt"]].astype(str), sep=":"
    )

    logger.info("Writing group file")
    with open(Path(out_dir) / "groups.txt", "w") as f:
        scored_variants = pd.merge(
            variants[["chrom_pos_ref_alt"]], scores, left_index=True, right_index=True
        )
        for k, v in tqdm(scored_variants.reset_index().groupby("gene_id")):
            f.write(f"{k} var " + " ".join(v["chrom_pos_ref_alt"]) + "\n")
            ...
            # TODO: Could also transform scores, e.g., beta(score, 25, 1)
            f.write(f"{k} weight " + " ".join(v["DeepRVAT_score"]) + "\n")

    ## Make BGEN file

    # Transpose genotype file to be variants x samples
    logger.info("Transforming genotype file")
    samples = list(pd.read_parquet("phenotype_file").index.astype(str))
    max_variant_id = variants["id"].max()
    with h5py.File(genotype_file, "r") as g:
        gt_matrix = g["gt_matrix"]
        variant_matrix = g["variant_matrix"]
        n_samples = gt_matrix.shape[0]
        max_samples = n_samples / 50  # TODO: Compute from max MAF
        sample_zarr = -zarr.ones(
            (max_variant_id + 1, max_samples), chunks=(1000, 1000), dtype=np.int8
        )
        gt_zarr = -zarr.ones(
            (max_variant_id + 1, max_samples), chunks=(1000, 1000), dtype=np.int8
        )
        last_filled = np.zeros(max_variant_id)
        for i in trange(n_samples):
            rows = variant_matrix[i, :]
            cols = last_filled[variant_matrix[i, :]]
            sample_zarr[(rows, cols)] = samples[i]
            gt_zarr[(rows, cols)] = gt_matrix[i, :]
            last_filled[rows] = cols + 1

    logger.info(f"Creating BGEN file")
    variants = (
        variants[["id", "chrom", "pos", "ref", "alt"]]
        .drop_duplicates(subset="id")
        .set_index("id")
    )
    with BgenWriter(
        Path(out_dir) / "genotypes.bgen",
        n_samples,
        samples=samples,
        metadata=f"Created from {genotype_file}",
    ) as f:
        for i in trange(max_variant_id + 1):
            if sample_zarr[i, 0] == -1:  # variant has no associated samples
                continue

            het_genotypes = np.zeros(n_samples, dtype=np.int8)
            homalt_genotypes = np.zeros(n_samples, dtype=np.int8)
            sparse_gt = gt_zarr[i]
            sparse_samples = sample_zarr[i]
            het_mask = sparse_gt == 1
            het_genotypes[sparse_samples[het_mask]] = 1
            hom_mask = sparse_gt == 2
            homalt_genotypes[sparse_samples[hom_mask]] = 1
            genotypes = np.stack(
                (
                    homalt_genotypes,
                    het_genotypes,
                    2 - (het_genotypes + homalt_genotypes),
                ),
                axis=1,
            )

            varid = variants.loc[i, "chrom_pos_ref_alt"]
            f.add_variant(
                varid=varid,
                rsid=varid,
                chrom=variants.loc[i, "chrom"],
...
@cli.command()
@click.option("--debug", is_flag=True)
@click.argument("data-config-file", type=click.Path(exists=True, path_type=Path))
@click.argument("model-config-file", type=click.Path(exists=True, path_type=Path))
@click.argument(
    "checkpoint-files", type=click.Path(exists=True, path_type=Path), nargs=-1
)
@click.argument("genotype-file", type=click.Path(exists=True, path_type=Path))
@click.argument("out-dir", type=click.Path(exists=True, path_type=Path))
def make_skat_input(
    debug: bool,
...
def score_variants(
    variants: pd.MultiIndex,
    model_config_file: PathLike,
    data_config_file: PathLike,
    ...
    :param variants: Pandas dataframe containing annotated variants
    ...
    #     + ["gene_id", "id"],
    # )
    # if variant_df["gene_id"].dtype == object:
    #     variant_df = (
    #         variant_df.loc[variant_df["gene_id"].apply(len) > 0]
    #         .explode("gene_id")
    #         .astype({"gene_id": int})
    #     )

    # logger.info("Selecting requested variants")
    # variant_df = (
    #     variant_df.set_index(["gene_id", "id"]).loc[variant_gene_ids].to_numpy()
    # )

    variants_np = variants.to_numpy()

    logger.info("Scoring variants using all ensemble models")
    n_variants = variants_np.shape[0]
    scores = np.zeros(n_variants)
    with torch.no_grad():
        for start_idx in trange(0, n_variants, batch_size, desc="Batches"):
            end_idx = min(start_idx + batch_size, n_variants)
            this_batch_size = end_idx - start_idx
            for checkpoint in tqdm(checkpoint_files, desc="Models"):
                if Path(str(checkpoint) + ".dropped").is_file():
                    # Ignore checkpoints that were chosen to be dropped
                    continue

                agg_model = load_one_model(model_config, checkpoint, device=device)
                if Path(str(checkpoint) + ".reverse").is_file():
                ...
                scores[start_idx:end_idx] += (
                    (
                        agg_model(
                            torch.tensor(
                                variants_np[start_idx:end_idx],
                                dtype=torch.float,
                                device=device,
                            ).reshape((this_batch_size, 1, -1, 1))

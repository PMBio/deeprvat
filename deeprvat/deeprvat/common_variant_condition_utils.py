...
def get_significant_genes(alpha, 
    ...
    logger.info(f'reading association testing resultsf from {res_file} and re-doing multiple testing correction using {pval_correction_method}')
    res = pd.read_parquet(res_file)
    res = pval_correction(res, alpha = alpha, correction_type = pval_correction_method)


    sig_genes = res.query('significant')[['gene', 'pval', 'pval_corrected']].merge(gene_df)
...
def prepare_genotypes_per_gene(
    ...
    for gene_id in list(sig_genes.index):
        ...
        included_vars = variants.intersect(gene_annotation_expanded[gene_annotation_expanded.gene_id.str.startswith(gene_ensembl_id)])
        included_vars = included_vars.as_df()["var_name"].to_list() if included_vars else []

        if len(included_vars) > 0:
            logger.info(f'Loading genotypes for {len(included_vars)} variants in gene region')
            ...
            selected_genos = selected_genos\
                .merge(ref_ac_df, how = 'left', on = 'IID')
            selected_genos = selected_genos.rename(columns = {"IID": "sample"}).set_index("sample")
            ...
            logger.info('taking 2 - AC to  get minor allele counts since plink returns reference allele counts')
            selected_genos = 2 - selected_genos
            logger.info('summary of minor allele frquencies')


            logger.info((selected_genos.sum(axis = 0)/(2*len(selected_genos))).describe())
    ...
    logger.info(f'Genes with no variants: {len(genes_with_no_variants), genes_with_no_variants}')
